...
export function DashboardLive({ locale }: { locale: Locale }) {
  ...
  const dateFormatter = useMemo(() => new Intl.DateTimeFormat(locale, { hour: "2-digit", minute: "2-digit" }), [locale]);

  const load = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const coreResponse = await fetch("/api/runtime/v1/core", { credentials: "include", cache: "no-store" });
      ...
      if (!reservationRoles.has(coreBody.data.currentRole)) {
        setReservations([]);
        return;
      }

      const location = coreBody.data.locations.find((item) => item.isActive);
      if (!location) { setReservations([]); return; }

      const from = new Date();
      from.setHours(0, 0, 0, 0);
      const to = new Date(from);
      to.setDate(to.getDate() + 1);
      const params = new URLSearchParams({ locationId: location.id, from: from.toISOString(), to: to.toISOString(), limit: "200" });
      const reservationResponse = await fetch(`/api/runtime/v1/reservations?${params.toString()}`, { credentials: "include", cache: "no-store" });
  ...
  if (loading && !core) return <main className="grid min-h-[70vh] place-items-center px-6"><p className="text-sm text-[var(--mandys-foreground-muted)]">{c.loading}</p></main>;

  if (error && !core) {
    return <main className="grid min-h-[70vh] place-items-center px-6"><div className="max-w-md rounded-[var(--mandys-radius-lg)] border border-[var(--mandys-border)] bg-[var(--mandys-surface)] p-6 text-center"><p className="text-sm text-[var(--mandys-danger)]">{error}</p><button className="mt-4 rounded-xl border border-[var(--mandys-border)] px-4 py-2 text-sm font-medium" onClick={() => void load()}>{c.retry}</button></div></main>;
  }
  if (!core) return null;

  const canSeeReservations = reservationRoles.has(core.currentRole);
  const canSeeEvents = eventRoles.has(core.currentRole);
  const activeReservations = canSeeReservations ? reservations.filter((item) => !["cancelled", "no_show"].includes(item.status)) : [];
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-[1500px] px-4 py-5 sm:px-6 lg:px-8">
      <header className="mb-8 flex flex-col gap-5 border-b border-[var(--mandys-border)] pb-6 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--mandys-accent)]">Mandy&apos;s</p><h1 className="text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">{restaurantName}</h1><p className="mt-2 max-w-2xl text-sm text-[var(--mandys-foreground-muted)]">{c.subtitle}</p></div>
        <div className="flex flex-wrap items-center gap-2 text-xs text-[var(--mandys-foreground-muted)]"><span>{localeLabels[locale]}</span><span aria-hidden="true">·</span><span>{c.role}: {core.currentRole}</span></div>
      </header>

      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <article className="rounded-[var(--mandys-radius-md)] border border-[var(--mandys-border)] bg-[var(--mandys-surface)] p-5 shadow-[var(--mandys-shadow-sm)]"><p className="text-2xl font-semibold tracking-[-0.03em]">{canSeeReservations ? activeReservations.length : "—"}</p><p className="mt-1 text-sm text-[var(--mandys-foreground-muted)]">{canSeeReservations ? c.todayReservations : c.unavailableForRole}</p></article>
        <article className="rounded-[var(--mandys-radius-md)] border border-[var(--mandys-border)] bg-[var(--mandys-surface)] p-5 shadow-[var(--mandys-shadow-sm)]"><p className="text-2xl font-semibold tracking-[-0.03em]">{canSeeReservations ? guests : "—"}</p><p className="mt-1 text-sm text-[var(--mandys-foreground-muted)]">{canSeeReservations ? c.guests : c.unavailableForRole}</p></article>
        ...
        <article className="rounded-[var(--mandys-radius-md)] border border-[var(--mandys-border)] bg-[var(--mandys-surface)] p-5 shadow-[var(--mandys-shadow-sm)]"><p className="text-2xl font-semibold tracking-[-0.03em]">{canSeeReservations && nextReservation ? dateFormatter.format(new Date(nextReservation.startsAt)) : "—"}</p><p className="mt-1 text-sm text-[var(--mandys-foreground-muted)]">{canSeeReservations ? c.next : c.unavailableForRole}</p></article>
      </section>

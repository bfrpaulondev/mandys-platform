...
const copy = {
  "pt-PT": { subtitle: "Histórico imutável das alterações relevantes no restaurante. Útil para segurança, suporte e saber quem fez o quê.", today: "Hoje", last24: "Últimas 24h", team7d: "Ações da equipa · 7d", public7d: "Ações públicas · 7d", source: "Origem", allSources: "Todas", team: "Equipa", public: "Website", system: "Sistema", action: "Ação", allActions: "Todas as ações", entity: "Área", allEntities: "Todas as áreas", loading: "A carregar atividade…", empty: "Ainda não existem registos com estes filtros.", refresh: "Atualizar", more: "Carregar mais", metadata: "Detalhes técnicos", actor: "por", publicActor: "Visitante do website", systemActor: "Sistema Mandy's" },
  "pt-BR": { subtitle: "Histórico imutável das alterações relevantes no restaurante. Útil para segurança, suporte e saber quem fez o quê.", today: "Hoje", last24: "Últimas 24h", team7d: "Ações da equipe · 7d", public7d: "Ações públicas · 7d", source: "Origem", allSources: "Todas", team: "Equipe", public: "Website", system: "Sistema", action: "Ação", allActions: "Todas as ações", entity: "Área", allEntities: "Todas as áreas", loading: "Carregando atividade…", empty: "Ainda não existem registros com estes filtros.", refresh: "Atualizar", more: "Carregar mais", metadata: "Detalhes técnicos", actor: "por", publicActor: "Visitante do website", systemActor: "Sistema Mandy's" },
  en: { subtitle: "An immutable history of meaningful restaurant changes. Useful for security, support and knowing who changed what.", today: "Today", last24: "Last 24h", team7d: "Team actions · 7d", public7d: "Public actions · 7d", source: "Source", allSources: "All", team: "Team", public: "Website", system: "System", action: "Action", allActions: "All actions", entity: "Area", allEntities: "All areas", loading: "Loading activity…", empty: "There are no records matching these filters yet.", refresh: "Refresh", more: "Load more", metadata: "Technical details", actor: "by", publicActor: "Website visitor", systemActor: "Mandy's system" },
  es: { subtitle: "Historial inmutable de los cambios relevantes del restaurante. Útil para seguridad, soporte y saber quién hizo qué.", today: "Hoy", last24: "Últimas 24h", team7d: "Acciones del equipo · 7d", public7d: "Acciones públicas · 7d", source: "Origen", allSources: "Todas", team: "Equipo", public: "Web", system: "Sistema", action: "Acción", allActions: "Todas las acciones", entity: "Área", allEntities: "Todas las áreas", loading: "Cargando actividad…", empty: "Todavía no hay registros con estos filtros.", refresh: "Actualizar", more: "Cargar más", metadata: "Detalles técnicos", actor: "por", publicActor: "Visitante de la web", systemActor: "Sistema Mandy's" },
...
export function ActivityBoard({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="space-y-6">
      ...
      : <section className="overflow-hidden rounded-[var(--mandys-radius-lg)] border border-[var(--mandys-border)] bg-[var(--mandys-surface)]"><div className="divide-y divide-[var(--mandys-border)]">{items.map((item) => {
          const actor = item.actorName ?? item.actorEmail ?? (item.source === "public" ? c.publicActor : c.systemActor);
          return <article key={item.id} className="p-4 sm:p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold">{readableAction(item.action, locale)}</h3><span className="rounded-full bg-[var(--mandys-surface-muted)] px-2 py-1 text-[11px] font-medium text-[var(--mandys-foreground-muted)]">{item.source === "team" ? c.team : item.source === "public" ? c.public : c.system}</span></div><p className="mt-1 text-sm text-[var(--mandys-foreground-muted)]">{readableEntity(item.entityType)}{item.entityId ? ` · ${item.entityId.slice(0, 12)}` : ""} · {c.actor} {actor}</p></div><time className="shrink-0 text-xs text-[var(--mandys-foreground-muted)]" dateTime={item.createdAt}>{dateFormatter.format(new Date(item.createdAt))}</time></div>{Object.keys(item.metadata).length > 0 ? <details className="mt-3"><summary className="cursor-pointer text-xs font-medium text-[var(--mandys-foreground-muted)]">{c.metadata}</summary><pre className="mt-2 max-h-72 overflow-auto rounded-md bg-[var(--mandys-surface-muted)] p-3 text-[11px] leading-5">{JSON.stringify(item.metadata, null, 2)}</pre></details> : null}</article>;
        })}</div>{nextCursor ? <div className="border-t border-[var(--mandys-border)] p-4 text-center"><Button variant="secondary" size="sm" disabled={loadingMore} onClick={() => void fetchPage(nextCursor, true)}>{c.more}</Button></div> : null}</section>}
    </div>
  );
}

...
export function OnboardingForm({ locale }: { locale: Locale }) {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const countryCode = String(form.get("countryCode") ?? regional.countryCode).trim().toUpperCase();
      const timezone = String(form.get("timezone") ?? regional.timezone).trim();
      const currency = String(form.get("currency") ?? regional.currency).trim().toUpperCase();
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6 rounded-[var(--mandys-radius-lg)] border border-[var(--mandys-border)] bg-[var(--mandys-surface)] p-5 shadow-[var(--mandys-shadow-sm)] sm:p-7">
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <label><span className="mb-1.5 block text-sm font-medium">{c.country}</span><input name="countryCode" defaultValue={regional.countryCode} required minLength={2} maxLength={2} pattern="[A-Za-z]{2}" className={`${inputClass} uppercase`} /></label>
        <label><span className="mb-1.5 block text-sm font-medium">{c.currency}</span><input name="currency" defaultValue={regional.currency} required minLength={3} maxLength={3} pattern="[A-Za-z]{3}" className={`${inputClass} uppercase`} /></label>
        <label className="sm:col-span-2"><span className="mb-1.5 block text-sm font-medium">{c.timezone}</span><input name="timezone" defaultValue={regional.timezone} required minLength={1} maxLength={80} className={`${inputClass} font-mono`} /></label>
      </div>
      ...
      <button type="submit" disabled={pending || !slug || enabledLocales.length === 0} className="w-full rounded-xl bg-[var(--mandys-foreground)] px-4 py-3 text-sm font-semibold text-white transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60">{pending ? "…" : c.submit}</button>
    </form>
  );
}

...
async function proxy(request: Request, context: RouteContext): Promise<Response> {
  ...
  const targetUrl = new URL(`${RESERVATIONS_UPSTREAM}/${all.map(encodeURIComponent).join("/")}`);
  ...
  try {
    ...
    return Response.json(
      { error: "RESERVATIONS_RUNTIME_UNAVAILABLE", message: "Mandy's reservations runtime is temporarily unavailable" },
      { status: 503 },
    );
  }
}

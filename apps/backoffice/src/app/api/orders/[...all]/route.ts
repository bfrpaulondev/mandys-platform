...
async function proxy(request: Request, context: RouteContext): Promise<Response> {
  ...
  const targetUrl = new URL(`${ORDERS_UPSTREAM}/${all.map(encodeURIComponent).join("/")}`);
  ...
  try {
    ...
    return Response.json({ error: "ORDERS_RUNTIME_UNAVAILABLE", message: "Mandy's orders runtime is temporarily unavailable" }, { status: 503 });
  }
}

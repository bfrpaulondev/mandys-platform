...
async function proxy(request: Request, context: RouteContext): Promise<Response> {
  ...
  const targetUrl = new URL(`${CRM_UPSTREAM}/${all.map(encodeURIComponent).join("/")}`);
  ...
  try {
    ...
    return Response.json({ error: "CRM_RUNTIME_UNAVAILABLE", message: "Mandy's CRM runtime is temporarily unavailable" }, { status: 503 });
  }
}

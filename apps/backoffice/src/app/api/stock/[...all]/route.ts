...
async function proxy(request: Request, context: RouteContext): Promise<Response> {
  ...
  const targetUrl = new URL(`${STOCK_UPSTREAM}/${all.map(encodeURIComponent).join("/")}`);
  ...
  try {
    ...
    return Response.json({ error: "STOCK_RUNTIME_UNAVAILABLE", message: "Mandy's stock runtime is temporarily unavailable" }, { status: 503 });
  }
}

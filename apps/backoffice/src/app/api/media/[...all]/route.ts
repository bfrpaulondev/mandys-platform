const MEDIA_UPSTREAM =
  "https://dbfmjdissqsdhxhmqkqp.supabase.co/functions/v1/mandys-media";
...
async function proxy(request: Request, context: RouteContext) {
  ...
  const targetUrl = new URL(`${MEDIA_UPSTREAM}/${all.map(encodeURIComponent).join("/")}`);
    ...
    return Response.json(
      {
        ...
        message: "Mandy's media runtime is temporarily unavailable",
      },
      { status: 503 },
    );
  }

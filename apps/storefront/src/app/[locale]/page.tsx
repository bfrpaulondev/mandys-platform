...
export async function generateMetadata({
  ...
  const title = `${data.restaurant.publicName} · Mandy's`;
  return {
    title,
...
export default async function RestaurantPage({
  ...
  return (
    <main
      style={themeStyle}
      className="min-h-screen bg-[var(--mandys-background)] text-[var(--mandys-foreground)]"
    >
      <nav className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-5 sm:px-8">
        <a href={`/${locale}`} className="text-lg font-semibold tracking-[-0.04em]">
          {data.restaurant.publicName}
        </a>
        <div className="flex items-center gap-3">
          <details className="relative text-sm">
            <summary className="cursor-pointer list-none text-[var(--mandys-foreground-muted)]">
              {localeLabels[locale]}
            </summary>
            <div className="absolute right-0 z-20 mt-3 min-w-48 rounded-[var(--mandys-radius-sm)] border border-[var(--mandys-border)] bg-[var(--mandys-surface)] p-2 shadow-lg">
              {data.enabledLocales.map((item) => (
                <a
                  key={item}
                  href={`/${item}`}
                  className="block rounded-md px-3 py-2 hover:bg-[var(--mandys-surface-muted)]"
                >
                  {localeLabels[item]}
                </a>
              ))}
          ...
          <a
            href="#booking"
            className="inline-flex min-h-11 items-center justify-center rounded-[var(--mandys-radius-sm)] bg-[var(--mandys-accent)] px-4 text-sm font-semibold text-[var(--mandys-accent-foreground)] transition hover:brightness-95"
          >
            {c.reserve}
          </a>
        </div>
      ...
      <section className="mx-auto grid min-h-[68vh] max-w-7xl items-center gap-10 px-5 py-16 sm:px-8 lg:grid-cols-[1.15fr_0.85fr] lg:py-24">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--mandys-accent)]">
              {c.eyebrow}
            </p>
            {data.isDemo ? (
              <span className="rounded-full border border-[var(--mandys-border)] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-[var(--mandys-foreground-muted)]">
                {c.demo}
              </span>
            ) : null}
          </div>
          <h1 className="mt-5 max-w-4xl text-5xl font-semibold tracking-[-0.055em] sm:text-6xl lg:text-7xl">
            {data.restaurant.publicName}
          </h1>
          <p className="mt-6 max-w-2xl text-base leading-7 text-[var(--mandys-foreground-muted)] sm:text-lg">
            {data.restaurant.description ?? c.titleFallback}
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            <a
              href="#booking"
              className="inline-flex min-h-12 items-center justify-center rounded-[var(--mandys-radius-sm)] bg-[var(--mandys-accent)] px-5 text-sm font-semibold text-[var(--mandys-accent-foreground)] transition hover:brightness-95"
            >
              {c.reserve}
            </a>
            <a
              href="#menu"
              className="inline-flex min-h-12 items-center justify-center rounded-[var(--mandys-radius-sm)] border border-[var(--mandys-border)] px-5 text-sm font-semibold transition hover:bg-[var(--mandys-surface-muted)]"
            >
              {c.menu}
            </a>
          </div>
        ...
        <aside className="rounded-[var(--mandys-radius-lg)] border border-[var(--mandys-border)] bg-[var(--mandys-surface)] p-6 shadow-[var(--mandys-shadow-sm)] sm:p-8">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--mandys-accent)]">
            {data.location.name}
          </p>
          {address.length > 0 ? (
            <address className="mt-4 not-italic text-sm leading-6 text-[var(--mandys-foreground-muted)]">
            ...
            <div className="mt-3 grid gap-2 text-sm">
              {data.openingHours.map((row) => (
                <div key={row.weekday} className="flex items-center justify-between gap-4 text-[var(--mandys-foreground-muted)]">
                  <span>{weekdays[locale][row.weekday] ?? row.weekday}</span>
                  <span>{row.isClosed ? c.closed : `${row.opensAt}–${row.closesAt}`}</span>
                </div>
              ))}
            </div>
          </div>
          {(data.restaurant.contactPhone || data.restaurant.contactEmail) ? (
            <div className="mt-7 border-t border-[var(--mandys-border)] pt-5">
              <p className="text-sm font-semibold">{c.contact}</p>
              <div className="mt-2 space-y-1 text-sm text-[var(--mandys-foreground-muted)]">
      ...
      <section id="menu" className="scroll-mt-6 border-t border-[var(--mandys-border)] bg-[var(--mandys-surface)]">
        <div className="mx-auto max-w-7xl px-5 py-16 sm:px-8 lg:py-20">
          ...
          {data.menus.length === 0 ? (
            <p className="mt-5 text-sm text-[var(--mandys-foreground-muted)]">{c.noMenu}</p>
          ) : (
            <div className="mt-8 space-y-16">
              {data.menus.map((menu) => (
                <div key={menu.id}>
                  ...
                  {menu.description ? (
                    <p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--mandys-foreground-muted)]">{menu.description}</p>
                  ) : null}
                  <div className="mt-9 space-y-12">
                    {menu.categories.map((category) => (
                      <section key={category.id}>
                        <div className="border-b border-[var(--mandys-border)] pb-3">
                          <h3 className="text-xl font-semibold">{category.name}</h3>
                          {category.description ? <p className="mt-1 text-sm text-[var(--mandys-foreground-muted)]">{category.description}</p> : null}
                        </div>
                              ...
                              <div className="flex items-start justify-between gap-4">
                                <div>
                                  <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    {item.isFeatured ? <span aria-label="featured" className="h-1.5 w-1.5 rounded-full bg-[var(--mandys-accent)]" /> : null}
                                  </div>
                                  {item.description ? (
                                    <p className="mt-2 text-sm leading-6 text-[var(--mandys-foreground-muted)]">{item.description}</p>
                                  ) : null}
                                </div>
                                <span className="whitespace-nowrap text-sm font-semibold">{money.format(item.priceCents / 100)}</span>
                              </div>
                              {item.allergens.length > 0 ? (
                                <p className="mt-3 text-xs text-[var(--mandys-foreground-muted)]">
                                  {c.allergens}: {item.allergens.map((allergen) => allergen.name).join(", ")}
                                </p>
                              ) : null}
                            </article>
                          ))}
      ...
      <section id="booking" className="scroll-mt-6 border-t border-[var(--mandys-border)]">
        <div className="mx-auto grid max-w-7xl gap-10 px-5 py-16 sm:px-8 lg:grid-cols-[0.75fr_1.25fr] lg:py-20">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--mandys-accent)]">Mandy&apos;s Reserve</p>
            <h2 className="mt-4 text-3xl font-semibold tracking-[-0.04em]">{c.bookingTitle}</h2>
            <p className="mt-4 max-w-lg text-sm leading-6 text-[var(--mandys-foreground-muted)]">{c.bookingText}</p>
          </div>
          <div className="rounded-[var(--mandys-radius-lg)] border border-[var(--mandys-border)] bg-[var(--mandys-surface)] p-5 shadow-[var(--mandys-shadow-sm)] sm:p-7">
            <ReservationForm locale={locale} disabled={data.isDemo} />
          </div>
        </div>
      </section>

      <footer className="border-t border-[var(--mandys-border)] px-5 py-8 text-center text-xs text-[var(--mandys-foreground-muted)] sm:px-8">
        Powered by Mandy&apos;s · {data.restaurant.publicName}
      </footer>
    </main>
  );
}

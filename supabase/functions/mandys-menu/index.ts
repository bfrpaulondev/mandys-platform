...
function canWrite(ctx: Context) { return ctx.role === "owner" || ctx.role === "manager"; }
...
async function createMenu(ctx: Context, b: any): Promise<Result> {
  if (!canWrite(ctx)) return fail(403,"FORBIDDEN","Only owners and managers can edit menus");
